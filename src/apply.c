...
struct obj *obj;
{
    ...
    if (obj->cursed && !rn2(2)) {
        ...
        You("你拍了一张 %s %s 的照片", s_suffix(mon_nam(u.ustuck)),
...
struct obj *obj;
{
    ...
    if (!freehand()) {
        ...
        case 1:
            if (!ublindf) {
                old = u.ucreamed;
                u.ucreamed += rn1(10, 3);
                pline("哟!你的 %s %s 更加有粘性了!", body_part(FACE),
                      (old ? "更加" : "现在有"));
    ...
    Your("%s 和 %s 已经干净了", body_part(FACE),
...
int rx, ry, *resp;
{
    ...
    if (!corpse && !statue) {
        ...
        if (!corpse) {
            ...
            Sprintf(buf, "%s 凉凉了", genders[gndr].he); /* "he"/"she"/"it" */
            buf[0] = highc(buf[0]);
        } else { /* plural */
            Strcpy(buf, "他们都凉凉了");
        }
        /* variations on "He's dead, Jim." (Star Trek's Dr McCoy) */
        You_hear("有个声音说, \"%s, Jim.\"", buf);
        *resp = 1;
        return TRUE;
        ...
        You("断定 %s 不幸的人 %s %s%s 死了",
            one ? (here ? "这个" : "那个") : (here ? "这些" : "那些"),
            one ? "" : "s", one ? "is" : "are", reviver ? " 大部分都" : "");
        return TRUE;
        ...
        pline("对于 %s 来说健康状况 %s", what, how);
        return TRUE;
    }
    return FALSE; /* no corpse or statue */
}

static const char hollow_str[] = "一个空洞的声音说,这一定是暗 %s!";
